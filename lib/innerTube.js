...
function extractAudioStreams(playerResponse) {
  ...
  const audioStreams = allFormats
    ...
    .map((f) => ({
      ...
      url: f.url || null, // For streaming directly
      hasCleanUrl: !!f.url,
    }))
    // Sort by preference
    .sort((a, b) => {
...
async function getAudioStream(videoInput, preferredItag) {
  ...
  if (preferredItag) {
    stream = info.audioStreams.find((s) => s.itag === preferredItag && s.url);
  }

  if (!stream) {
    stream = info.audioStreams.find((s) => s.url);
  }

  if (!stream) {
    throw new Error("No direct audio stream URL available");
  }
...
async function getPlaylistInfo(playlistInput) {
  ...
  const metadata = data?.sidebar?.playlistSidebarRenderer?.items || [];
  ...
  for (const section of contents) {
    const items = section?.playlistVideoListRenderer?.contents || [];
